...
private:
    ...
    FibNode<ElementType>* Find(FibNode<ElementType>* root, int key);

    // Find node of key "int key" in heap.
    FibNode<ElementType>* Find(ElementType key);
...
template <class ElementType>
FibNode<ElementType>* FibHeap<ElementType>::Find(FibNode<ElementType>* T, int key)
{
    ...
    do
    {
        ...
        {
            if ((res = Find(cur->child, key)) != nullptr)
                break;
        }
        cur = cur->right;
    } while (cur != T);
...
// Find node of key "int key" in heap.
template <class ElementType>
FibNode<ElementType>* FibHeap<ElementType>::Find(ElementType key)
{
    if (m_min == nullptr)
        return nullptr;

    return Find(m_min, key);
}
...
// Get min tree.
template <class ElementType>
FibHeap<ElementType>* FibHeap<ElementType>::GetMin()
{
    return m_min;
}
...
{
    return Find(key) != nullptr ? true : false;
}
